#### ROBUSTNESS CHECK - EXPONENTIAL DECAY ####
...
appariement_emplois = TRUE #false
car_only = FALSE
...
for (i in 1:(length(listYearLagged) - 2)){
  ...
  com_temp <- tt %>% 
    ...
    gather('group', 'emp_final', group2:group5) %>%
    ...
    summarise(sum_jobs_init = sum(emp_init, na.rm = TRUE),
              ...
              access_emp_final_tt_final_40 = sum(emp_final * exp(-0.0692 * time_final), na.rm = TRUE), #/sum_jobs_final,
              access_emp_init_tt_init_40 = sum(emp_init * exp(-0.0692 * time_init), na.rm = TRUE), #/sum_jobs_init,
              access_emp_after_tt_after_40 = sum(emp_after * exp(-0.0692 * time_after), na.rm = TRUE), #/sum_jobs_init,
...
for (i in 1:(length(listYearLagged) - 2)){
  ...
  com_temp <- tt %>% 
    ...
    summarise(sum_jobs_init = sum(emp_init, na.rm = TRUE),
              sum_jobs_final = sum(emp_final, na.rm = TRUE),
              sum_jobs_after = sum(emp_after, na.rm = TRUE),
              access_emp_final_tt_final_40 = sum(emp_final * (sum_jobs_init / sum_jobs_final) * exp(-0.0692 * time_final), na.rm = TRUE), #/sum_jobs_final,
              access_emp_after_tt_after_40 = sum(emp_after* (sum_jobs_final / sum_jobs_after) * exp(-0.0692 * time_after), na.rm = TRUE), #/sum_jobs_init,
    ) %>% 
    dplyr::select(-sum_jobs_final, -sum_jobs_init, -sum_jobs_after) %>%
    data.frame()
  com_temp <- com_temp %>% mutate(year = listYearLagged[i])
...
for (i in 1:(length(listYearLagged) - 2)){
  ...
  com_temp <- tt %>% 
    ...
    summarise(sum_jobs_init = sum(emp_init, na.rm = TRUE),
              sum_jobs_final = sum(emp_final, na.rm = TRUE),
              sum_jobs_after = sum(emp_after, na.rm = TRUE),
              access_emp_final_tt_final_40 = sum(emp_init * exp(-0.0692 * time_final), na.rm = TRUE), #/sum_jobs_final,
              access_emp_after_tt_after_40 = sum(emp_final * exp(-0.0692 * time_after), na.rm = TRUE), #/sum_jobs_init,
...
write.csv(df, paste0(path_outputs,'exponential_decay_additive.csv'))
write.csv(weighted_accessibility[,1:4], paste0(path_outputs,'exponential_decay_weighted_accessibility.csv'))
...
df = rbind(diff_jg, diff_speed, diff_proximity, diff_proximity_jobs, diff_proximity_hh)
write.csv(df, paste0(path_outputs,'exponential_decay_multiplicative.csv'))
